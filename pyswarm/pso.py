...
class Pso(object):
    ...
    def __init__(self, maxiter=100, minstep=1e-6, minfunc=1e-6,
        ...
        :param minfunc: scalar
            The minimum change of swarm's best objective value before
            the search terminates (Default: 1e-8)
        :param omega: scalar
            Particle velocity scaling factor (Default: 0.5)
        :param phip: scalar
            Scaling factor to search away from the particle's best
    ...
    def initialize(self, func, lb, ub, ieqcons=[], f_ieqcons=None,
        ...
        :param f_ieqcons: function
            ...
            to 0.0 in a successfully optimized problem.
            If f_ieqcons is specified, ieqcons is ignored (Default: None)
        :param args: tuple
    ...
    def optimize(self, swarmsize=100, x0=None, processes=1):
        ...
        :param processes: int
            ...
            = 0: the set of particles is given to the cost function self.func
                for a user implemented parallelism
        ...
        if self.__ready is False:
            if self.debug:
                ...
                logging.debug('\tRun the "initialize"\
                                    method before optimizing.')
            return None
        if len(self.lb) != len(self.ub):
            logging.debug('Lower- and upper-bounds must be the same length')
        if hasattr(self.func, '__call__') is False:
            logging.debug('Invalid function handle')
        self.lb = np.array(self.lb)
        self.ub = np.array(self.ub)
        if np.all(self.ub > self.lb) is True:
            logging.debug('All upper-bound values must be\
        ...
        self.obj = \
            partial(self._obj_wrapper, self.func, self.args, self.kwargs)
        ...
        if self.f_ieqcons is None:
            if not len(self.ieqcons):
                ...
                if self.debug:
                    logging.debug('Converting ieqcons to\
                     a single constraint function')
                self.cons = partial(self._cons_ieqcons_wrapper,
                                    self.ieqcons, self.args, self.kwargs)
        else:
            if self.debug:
                logging.debug('Single constraint function given in f_ieqcons')
            self.cons = partial(self._cons_f_ieqcons_wrapper, self.f_ieqcons,
        ...
        if fp[i_min] < fg:
            ...
            # so just give it a temporary "best" point since
        ...
        while it <= self.maxiter:
            ...
            if fp[i_min] < fg:
                ...
                if np.abs(fg - fp[i_min]) <= self.minfunc:
                    if self.verbose:
                        logging.info('Stopping search: Swarm best objective\
                                change less than {:}'.format(self.minfunc))
                    if self.particle_output:
                        return{'optimal particle': p_min,
                               'optimal function': fp[i_min],
                               'convergence particles': p_save,
                               'convergence functions': fp_save}
                    else:
                        return p_min, fp[i_min]
                elif stepsize <= self.minstep:
                    if self.verbose:
                        logging.info('Stopping search: \
                Swarm best position change less than {:}'.format(self.minstep))
                    if self.particle_output:
                        return{'optimal particle': p_min,
                               'optimal function': fp[i_min],
                               'convergence particles': p_save,
            ...
            if self.verbose:
                logging.info(
                    'Best after iteration {:}: {:} {:}'.format(it, g, fg))
            it += 1
        ...
        if not self.is_feasible(g):
            if self.verbose:
                logging.info("However, the optimization couldn't find\
                 a feasible design. Sorry")
        if self.particle_output:
            if self.verbose:
